...
const LoginForm: React.FC<TLoginFormProps> = ({ onSubmit, startupConfig, error, setError }) => {
  ...
  const authInputClassName =
    'webkit-dark-styles transition-color peer w-full rounded-2xl border border-border-light bg-surface-primary px-3.5 pb-2.5 pt-3 text-text-primary duration-200 hover:border-border-light focus:border-brand-purple focus:outline-none focus-visible:border-brand-purple';
  const authSecretInputClassName = `${authInputClassName} h-auto pr-12`;
  const authLabelClassName =
    'absolute start-3 top-1.5 z-10 origin-[0] -translate-y-4 scale-75 transform bg-surface-primary px-2 text-sm text-text-secondary-alt duration-200 peer-placeholder-shown:top-1/2 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:scale-100 peer-focus:top-1.5 peer-focus:-translate-y-4 peer-focus:scale-75 peer-focus:px-2 peer-focus:text-brand-purple dark:peer-focus:text-brand-purple rtl:peer-focus:left-auto rtl:peer-focus:translate-x-1/4';
  ...
  return (
    <>
      {showResendLink && (
        <div className="mt-2 rounded-md border border-brand-purple-subtle-border bg-brand-purple-subtle px-3 py-2 text-sm text-gray-600 dark:text-gray-200">
